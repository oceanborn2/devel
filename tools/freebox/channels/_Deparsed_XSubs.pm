...
sub SetFNameFilter(;$) ;
sub SetFileCallback(;$) ;
sub SetFileNameCallback(;$) ;
sub SetMsgCallback(;$) ;
sub SetOption($$) ;
sub Smerge($) ;
sub strerror($) ;
}
package Convert::UUlib::Item {
sub binfile($) ;
sub decode($;$) ;
sub decode_temp($) ;
sub filename($;$) ;
...
sub dumpPerlRefCount;
sub gettimeofday() ;
sub preloadSelectors;
sub releaseObjectiveCObject;
sub releaseProxyForPerlObject;
sub releaseStruct;
sub sendObjcMessage;
sub setBridgeTracing;
}
