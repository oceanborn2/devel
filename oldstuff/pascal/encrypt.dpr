...
Type
    Chrono = Record
     ...
     Banner = Object
              ...
              Z       : Char;    {caractŠre interm‚diaire}
              VChem   : String;  {ancien chemin}
              Chemin  : DirStr;  {chemin d'accŠs au fichier}
...
{passe une chaine de caractŠres en majuscules}
...
Begin
     ...
     Writeln (' CD .. m‚thode complŠte, d‚cryptage');
     Writeln (' CC .. m‚thode complŠte, cryptage');
     Writeln;
     Writeln (' Si vous trouvez ce logiciel utile et que vous vous en servez r‚guliŠrement,');
     Writeln (' alors veuillez envoyer 50 FF par chŠque … l''adresse suivante :');
     Writeln;
...
Begin
     ...
     Writeln ('  ceci avec les paramŠtres programm‚s, … savoir : ');
     Writeln;
...
Begin
     ...
     Writeln ('  fichiers respectant la syntaxe du systŠme d''exploitation.');
     Writeln;
...
Begin
     ...
     Writeln ('   - la m‚thode complŠte       : m‚thode pr‚c‚dente am‚lior‚e');
     Writeln;
     Writeln ('  la m‚thode traditionnelle est la plus rapide mais aussi la moins');
     Writeln ('  efficace, contrairement … la m‚thode complŠte qui est la plus ');
     Writeln ('  lente mais qui chiffre les donn‚es de maniŠre trŠs complexe. ');
     Writeln;
     Writeln ('  la m‚thode complŠte se s‚pare en une m‚thode de cryptage (CC)');
...
Begin
     ...
     Writeln ('  ne doit pas ˆtre prise … la l‚gŠre.');
     Writeln;
     Writeln ('  le fichier crypt‚ ne contient bien ‚videmment pas ce mot de passe,');
     Writeln ('  il sert simplement au calcul des nombres n‚c‚ssaires au cryptage.');
     Writeln;
     Writeln ('  Attention : le programme fait la diff‚rence entre les minuscules');
     Writeln ('  et les majuscules. Le cryptage ne donne pas les mˆmes r‚sultats.');
     Ch:=Readkey;
     CloseWindow;
     End;
...
Begin
     ...
     Writeln (' Û     Û Û Û Û     Û   Û Û   Û Û   Û   Û           c“ute que 50 FF et donne ');
     Writeln (' ÛÛ    Û Û Û Û     ÛÛÛÛ   ÛÛÛ  ÛÛÛÛÛ   Û           droit … la prochaine     ');
     Writeln (' Û     Û  ÛÛ Û     Û   Û   Û   Û       Û           version pour 15 FF.      ');
     Writeln (' ÛÛÛÛÛ Û   Û ÛÛÛÛÛ Û   Û   Û   Û       Û                                    ');
     Writeln ('                                          1.00     Pour cela, envoyez un    ');
     Writeln (' Logiciel de cryptage multi-m‚thodes               chŠque … cette adresse : ');
...
{     SetIntVec(1,Ptr(0,0));
     SetIntVec(3,Ptr(0,0));}
     TopWindow:=Nil; {pas de fenˆtres pour l'instant}
     P:=Nil; {P mis … NIL pour ‚viter un FREEMEM pr‚coce}
     TextMode(Co80);
     For I:=1 To 8 Do Fil[i]:='';
...
{crypte le dernier bloc - m‚thode complŠte}
...
{crypte un bloc - m‚thode complŠte}
...
{initialise le cryptage - m‚thode complŠte}
...
{d‚crypte le dernier bloc - m‚thode complŠte}
...
{d‚crypte un bloc - m‚thode complŠte}
...
{initialise le d‚cryptage - m‚thode complŠte}
