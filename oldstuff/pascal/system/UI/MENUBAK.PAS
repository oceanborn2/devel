...
Var
    ...
    Page_OFS      : Word absolute $40:$4E; {adresse d'offset de la page ‚cran}
...
Begin
    ...
    { regs.ah:=$08;
     ...
     {premiŠre m‚thode non bios}
     Off:=160*WhereY+2*WhereX+Page_OFS;
     C:=Char(mem[Vseg:Off]);
     A:=Char(mem[Vseg:Off+1]);
     End;
...
Begin
     GetMem(S,2*Taille+1);
     S^[0]:=Char(2*Taille);
     Off:=160*YY+2*XX+Page_Ofs;
     Move(Ptr(VSeg,Off)^,S^,Taille*2);
     End;
...
Begin
     ...
     Stat:=Norm;
     SAttr:=TextAttr;
     Off:=160*PY+2*PX+Page_Ofs;
     TextAttr:=AVN;
     Move(T^,Ptr(VSeg,Off)^,Length(T^));
     TextAttr:=HAVN;
     Gotoxy(PX+HPOS,PY);
     Write(T^[HPos+1]);
     TextAttr:=SAttr;
...
Begin
     If Initial <> Signature Then Exit;
     Offst:=160*PY+2*PX+Page_Ofs;
     Move(S^,Ptr(VSeg,Offst)^,Length(S^));
     End;
